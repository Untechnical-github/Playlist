...
- **YouTube Data API v3のクォータに注意**: `search.list`は1回100ユニット消費し、デフォルトの
  ...
  より2回目以降の実行は新規・境界線付近の曲だけで済むため、実際の消費はかなり減る）。
...
- 対象プレイリストの指定は不要。自分が所有するプレイリストのうち、**タイトルが「Playlist」で
  始まるもの**（集計先の「Playlist」自身や「Playlist II」等）と**「English Songs」**を除いた
  すべてが集計対象になる（`get_source_playlists`）
- `threshold`は「万回再生」単位。例: `100` → 100万回再生以上、`5000` → 5000万回再生以上、
  `20000` → 2億回再生以上の曲を対象にする
- **「Playlist」は毎回、今回のしきい値を満たす曲の集合と完全に同期する**:
  - まだ入っていない・条件を満たす曲は追加する
  - 既に入っているが、今回のしきい値を満たさなくなった曲（前回より厳しい条件を選んだ場合等）は
    自動的に削除される
  - 追加・削除どちらも今回実行分だけがメッセージに表示される（何も省略せず、長い場合は
    複数メッセージに分割される）
  - 注意: 「Playlist」はこのコマンドが完全に管理するプレイリストという前提。集計対象の曲と
    無関係な曲を手動で「Playlist」に入れている場合、次回実行時にしきい値を満たさなければ
    削除される
...
- **境界線付近の曲だけ再生回数を再取得する**: キャッシュに値がある曲でも、しきい値の80%以上
  ...
  節約のためにこの範囲だけ再取得する。上書き設定（後述）の候補アーティストについても、
  曲自体との最大値が境界線付近であれば同様に再取得する。
- **追加でチェックするアーティストの上書き設定（`score_overrides.json`）**: 曲名にキーの
  文字列が含まれていたら、値に列挙したアーティスト名それぞれでも再生回数を調べ、曲自体の
  再生回数と合わせた最大値をしきい値判定に使う（Playlistに追加されるのはあくまで曲自体＝
  クレジット通りの動画）。どれか1つでもしきい値を超えていればよい、という方針。想定するケース:
  - **カバー曲**: カバー動画自体の再生回数は少なくても、元の曲が有名なら含めたい場合
    （例: 星街すいせいの「少女レイ」カバー → 元曲のBUMP OF CHICKENの再生回数も候補に加える）
  - **同じ曲の複数の公式/コラボ動画**: 1つの曲に対して複数の公式動画がある場合
    （例: BUMP OF CHICKEN「アカシア」は本人名義の動画とは別にPokémon公式のコラボ動画があり、
    どちらか一方でも再生回数がしきい値を超えていれば対象にしたい）
  プレイリストをまたいで使う設定なので、`artist_groups.json`と違いプレイリストID単位ではなく
  フラットな対応表になっている。
  ```json
  {
    "曲名に含まれる文字列": ["追加でチェックするアーティスト名", "..."]
  }
  ```
