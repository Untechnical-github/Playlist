...
5. 以下を実行し、表示されるURLを開いてログイン・許可した後、ターミナルでEnterを押す
   ...
   基本的に再セットアップは不要。
   **ただし、Google Cloud ConsoleのOAuth同意画面が「テスト」公開ステータスのままだと、
   リフレッシュトークンは発行から7日で失効する**（Googleの仕様）。自動実行を長期間安定させたい
   場合は、OAuth同意画面を「本番環境」に公開しておくこと（自分だけが使う個人用アプリでも、
   Googleの確認レビューは通常不要。「このアプリは確認されていません」という警告は自分の
   アカウントで承認して進めれば問題ない）。7日で失効すると、`/score`実行時に
   `KeyError: 'access_token'`（リフレッシュ失敗）でジョブが落ちる。その場合は本手順を
   やり直して`auth/oauth.json`を再発行し、GitHub Actionsの`OAUTH_TOKEN_JSON`シークレットと
   Workerの`GOOGLE_REFRESH_TOKEN`シークレット（手順3）を新しい値に更新する。
...
- Worker/GitHub Actionsは `auth/oauth.json` / `auth/oauth_client.json` の中身をそのまま
  Secretsとして使う。OAuth同意画面を「本番環境」に公開していればリフレッシュトークンは
  長期間有効で自動更新されるため、Cookie方式と違って定期的な手動再取得は基本的に不要
  （「テスト」公開のままだと7日で失効するので要注意。詳細は前述のセットアップ手順を参照）。
