...
- **「塊」（同じアーティストの曲が2曲以上）だけをアルファベット順にまとめる**: 本当に2曲以上ある
  ...
  並べて前方にまとめる。
- **曲がそのアーティストとしては1曲しかない（＝実質単独タグの）曲は、位置（隣接している曲）で
  所属を決める**: 隣接する曲がどこかの塊に属していれば、その塊の一員として扱われる。隣接する
  曲が無い（他の単独曲に挟まれている、末尾で孤立している等）場合は、自分のタグのまま「単独曲」
  として塊とは別に、元の相対順で残る。
  - これにより、ユーザーがアプリ上で「YouTube限定動画やコラボ曲、あるいは普段この1曲しか
    無いアーティストの曲を、あるアーティストの隣に手動で置く」という操作をした場合、それが
    どの位置にあっても次回の `plan` で塊の一員として扱われる。専用の「上書き登録」は不要。
  - 注意点: これは「たまたま塊の隣にあっただけ」なのか「意図的に手動で置いた」のかを区別する
    手段が無いため、まだ一度もソートしていない・アーティストが入り乱れたプレイリストに対して
    実行すると、たまたま塊の隣にあっただけの無関係な単独曲まで巻き込まれることがある。
    さらに、単独曲同士が連続している場合は連鎖的に巻き込まれることもある（Aが塊の隣→Aが
    塊に取り込まれる→Aの隣にいたBもAに引き寄せられて取り込まれる、という連鎖）。
- **状態ファイルは持たない**: 前回実行時の情報を保存せず、常に現在のプレイリストの実際の並び
  だけを見て計算し直す。
- **コラボ曲・不明曲（UGC動画等）の扱い**: これらは単体では「どのアーティストに属するか」を
  機械的に決められないため、単独タグの曲と同様に**現在どの曲の隣に置かれているか**を見て、
  隣接している曲と同じアーティスト扱いにする。
  - コラボ曲は、掲載アーティストのいずれかが隣接する曲のアーティストと一致すればそちらに所属。
    一致する隣接がなければ、掲載順の先頭アーティストで確定する。
  - アーティスト情報が全く無い曲（UGC動画等）は、隣接するどちらかの曲とそのまま同じ扱いになる。
  - どの隣接曲とも同じ扱いにできなかった曲は上記の「単独曲」と同様に末尾へ、元の相対順のまま
    送られる。
...
- **カタカナ表記とローマ字表記の自動統合**: "ヨルシカ" と "Yorushika" のような、カタカナ表記と
  そのローマ字表記は`pykakasi`でカタカナをローマ字に変換して比較し、登録なしで自動的に同一
  アーティストとみなして統合する（`core/planner.py` の `_auto_merge_transliterations`）。
