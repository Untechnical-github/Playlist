...
- **カバー曲の元曲基準判定（`score_overrides.json`）**: カバー動画自体の再生回数は少なくても、
  元の曲が有名なら含めたい場合の上書き設定。曲名にキーの文字列が含まれていたら、しきい値の
  判定だけそのアーティスト名の再生回数を使う（Playlistに追加されるのはあくまでカバー動画自体）。
  プレイリストをまたいで使う設定なので、`artist_groups.json`と違いプレイリストID単位ではなく
  フラットな対応表になっている。
  ```json
  {
    "曲名に含まれる文字列": "判定に使うアーティスト名"
  }
  ```
